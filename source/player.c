...
bool can_move_target(TSprite *link){
	...
	bool GroundIsSteppable=( MetaTileEquals(pt.x,pt.y, 0x0001, g_bg.dstMap, inanimatesMetaTiles, 0) || MetaTileEquals(pt.x, pt.y, 0x0003, g_bg.dstMap, inanimatesMetaTiles, 0));
	bool WallNotThere=MetaTileEquals(pt.x,pt.y, 0x0000, g_walls.dstMap, inanimatesMetaTiles, 0);
...
void player_move(TSprite *link)
{
	...
	if(link->state==SPR_STATE_SWING){
		if(MetaTileEquals(pt.x,pt.y,0x05,g_walls.dstMap, inanimatesMetaTiles, 0)){
			MetaTileLoad(pt.x,pt.y,0x00,g_walls.dstMap, inanimatesMetaTiles, 0);
		}
		if(MetaTileEquals(pt.x,pt.y,0x08,g_walls.dstMap, inanimatesMetaTiles, 0)){
			MetaTileLoad(pt.x,pt.y,0x09,g_walls.dstMap, inanimatesMetaTiles, 0);
